/*
Sencai Platform API

Consolidated OpenAPI 3.x specification for the Sencai platform backend (`/api/v1/_*`). Covers auto-discovered CRUD content-types plus hand-documented custom routes. See `openapi/README.md` for regeneration instructions and versioning/deprecation policy (`SUNSET_DATE` env convention, see doc.sencai.space/dev/api-versioning).

API version: 1.0.0
Contact: api@sencai.space
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package sencaisdk

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the Nis2Gap type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Nis2Gap{}

// Nis2Gap struct for Nis2Gap
type Nis2Gap struct {
	Requirement string `json:"requirement"`
	Article *string `json:"article,omitempty"`
	Annex string `json:"annex"`
	GapDescription *string `json:"gap_description,omitempty"`
	Status *string `json:"status,omitempty"`
	TargetDate *string `json:"target_date,omitempty"`
	Owner *string `json:"owner,omitempty"`
	EvidenceLinks map[string]interface{} `json:"evidence_links,omitempty"`
	AuditActionEvidence map[string]interface{} `json:"audit_action_evidence,omitempty"`
	Organisation *CreateAccessReviewRequestDataReviewer `json:"organisation,omitempty"`
}

type _Nis2Gap Nis2Gap

// NewNis2Gap instantiates a new Nis2Gap object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNis2Gap(requirement string, annex string) *Nis2Gap {
	this := Nis2Gap{}
	this.Requirement = requirement
	this.Annex = annex
	return &this
}

// NewNis2GapWithDefaults instantiates a new Nis2Gap object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNis2GapWithDefaults() *Nis2Gap {
	this := Nis2Gap{}
	return &this
}

// GetRequirement returns the Requirement field value
func (o *Nis2Gap) GetRequirement() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Requirement
}

// GetRequirementOk returns a tuple with the Requirement field value
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetRequirementOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Requirement, true
}

// SetRequirement sets field value
func (o *Nis2Gap) SetRequirement(v string) {
	o.Requirement = v
}

// GetArticle returns the Article field value if set, zero value otherwise.
func (o *Nis2Gap) GetArticle() string {
	if o == nil || IsNil(o.Article) {
		var ret string
		return ret
	}
	return *o.Article
}

// GetArticleOk returns a tuple with the Article field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetArticleOk() (*string, bool) {
	if o == nil || IsNil(o.Article) {
		return nil, false
	}
	return o.Article, true
}

// HasArticle returns a boolean if a field has been set.
func (o *Nis2Gap) HasArticle() bool {
	if o != nil && !IsNil(o.Article) {
		return true
	}

	return false
}

// SetArticle gets a reference to the given string and assigns it to the Article field.
func (o *Nis2Gap) SetArticle(v string) {
	o.Article = &v
}

// GetAnnex returns the Annex field value
func (o *Nis2Gap) GetAnnex() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Annex
}

// GetAnnexOk returns a tuple with the Annex field value
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetAnnexOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Annex, true
}

// SetAnnex sets field value
func (o *Nis2Gap) SetAnnex(v string) {
	o.Annex = v
}

// GetGapDescription returns the GapDescription field value if set, zero value otherwise.
func (o *Nis2Gap) GetGapDescription() string {
	if o == nil || IsNil(o.GapDescription) {
		var ret string
		return ret
	}
	return *o.GapDescription
}

// GetGapDescriptionOk returns a tuple with the GapDescription field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetGapDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.GapDescription) {
		return nil, false
	}
	return o.GapDescription, true
}

// HasGapDescription returns a boolean if a field has been set.
func (o *Nis2Gap) HasGapDescription() bool {
	if o != nil && !IsNil(o.GapDescription) {
		return true
	}

	return false
}

// SetGapDescription gets a reference to the given string and assigns it to the GapDescription field.
func (o *Nis2Gap) SetGapDescription(v string) {
	o.GapDescription = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *Nis2Gap) GetStatus() string {
	if o == nil || IsNil(o.Status) {
		var ret string
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetStatusOk() (*string, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *Nis2Gap) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given string and assigns it to the Status field.
func (o *Nis2Gap) SetStatus(v string) {
	o.Status = &v
}

// GetTargetDate returns the TargetDate field value if set, zero value otherwise.
func (o *Nis2Gap) GetTargetDate() string {
	if o == nil || IsNil(o.TargetDate) {
		var ret string
		return ret
	}
	return *o.TargetDate
}

// GetTargetDateOk returns a tuple with the TargetDate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetTargetDateOk() (*string, bool) {
	if o == nil || IsNil(o.TargetDate) {
		return nil, false
	}
	return o.TargetDate, true
}

// HasTargetDate returns a boolean if a field has been set.
func (o *Nis2Gap) HasTargetDate() bool {
	if o != nil && !IsNil(o.TargetDate) {
		return true
	}

	return false
}

// SetTargetDate gets a reference to the given string and assigns it to the TargetDate field.
func (o *Nis2Gap) SetTargetDate(v string) {
	o.TargetDate = &v
}

// GetOwner returns the Owner field value if set, zero value otherwise.
func (o *Nis2Gap) GetOwner() string {
	if o == nil || IsNil(o.Owner) {
		var ret string
		return ret
	}
	return *o.Owner
}

// GetOwnerOk returns a tuple with the Owner field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetOwnerOk() (*string, bool) {
	if o == nil || IsNil(o.Owner) {
		return nil, false
	}
	return o.Owner, true
}

// HasOwner returns a boolean if a field has been set.
func (o *Nis2Gap) HasOwner() bool {
	if o != nil && !IsNil(o.Owner) {
		return true
	}

	return false
}

// SetOwner gets a reference to the given string and assigns it to the Owner field.
func (o *Nis2Gap) SetOwner(v string) {
	o.Owner = &v
}

// GetEvidenceLinks returns the EvidenceLinks field value if set, zero value otherwise.
func (o *Nis2Gap) GetEvidenceLinks() map[string]interface{} {
	if o == nil || IsNil(o.EvidenceLinks) {
		var ret map[string]interface{}
		return ret
	}
	return o.EvidenceLinks
}

// GetEvidenceLinksOk returns a tuple with the EvidenceLinks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetEvidenceLinksOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.EvidenceLinks) {
		return map[string]interface{}{}, false
	}
	return o.EvidenceLinks, true
}

// HasEvidenceLinks returns a boolean if a field has been set.
func (o *Nis2Gap) HasEvidenceLinks() bool {
	if o != nil && !IsNil(o.EvidenceLinks) {
		return true
	}

	return false
}

// SetEvidenceLinks gets a reference to the given map[string]interface{} and assigns it to the EvidenceLinks field.
func (o *Nis2Gap) SetEvidenceLinks(v map[string]interface{}) {
	o.EvidenceLinks = v
}

// GetAuditActionEvidence returns the AuditActionEvidence field value if set, zero value otherwise.
func (o *Nis2Gap) GetAuditActionEvidence() map[string]interface{} {
	if o == nil || IsNil(o.AuditActionEvidence) {
		var ret map[string]interface{}
		return ret
	}
	return o.AuditActionEvidence
}

// GetAuditActionEvidenceOk returns a tuple with the AuditActionEvidence field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetAuditActionEvidenceOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.AuditActionEvidence) {
		return map[string]interface{}{}, false
	}
	return o.AuditActionEvidence, true
}

// HasAuditActionEvidence returns a boolean if a field has been set.
func (o *Nis2Gap) HasAuditActionEvidence() bool {
	if o != nil && !IsNil(o.AuditActionEvidence) {
		return true
	}

	return false
}

// SetAuditActionEvidence gets a reference to the given map[string]interface{} and assigns it to the AuditActionEvidence field.
func (o *Nis2Gap) SetAuditActionEvidence(v map[string]interface{}) {
	o.AuditActionEvidence = v
}

// GetOrganisation returns the Organisation field value if set, zero value otherwise.
func (o *Nis2Gap) GetOrganisation() CreateAccessReviewRequestDataReviewer {
	if o == nil || IsNil(o.Organisation) {
		var ret CreateAccessReviewRequestDataReviewer
		return ret
	}
	return *o.Organisation
}

// GetOrganisationOk returns a tuple with the Organisation field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Nis2Gap) GetOrganisationOk() (*CreateAccessReviewRequestDataReviewer, bool) {
	if o == nil || IsNil(o.Organisation) {
		return nil, false
	}
	return o.Organisation, true
}

// HasOrganisation returns a boolean if a field has been set.
func (o *Nis2Gap) HasOrganisation() bool {
	if o != nil && !IsNil(o.Organisation) {
		return true
	}

	return false
}

// SetOrganisation gets a reference to the given CreateAccessReviewRequestDataReviewer and assigns it to the Organisation field.
func (o *Nis2Gap) SetOrganisation(v CreateAccessReviewRequestDataReviewer) {
	o.Organisation = &v
}

func (o Nis2Gap) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Nis2Gap) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["requirement"] = o.Requirement
	if !IsNil(o.Article) {
		toSerialize["article"] = o.Article
	}
	toSerialize["annex"] = o.Annex
	if !IsNil(o.GapDescription) {
		toSerialize["gap_description"] = o.GapDescription
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if !IsNil(o.TargetDate) {
		toSerialize["target_date"] = o.TargetDate
	}
	if !IsNil(o.Owner) {
		toSerialize["owner"] = o.Owner
	}
	if !IsNil(o.EvidenceLinks) {
		toSerialize["evidence_links"] = o.EvidenceLinks
	}
	if !IsNil(o.AuditActionEvidence) {
		toSerialize["audit_action_evidence"] = o.AuditActionEvidence
	}
	if !IsNil(o.Organisation) {
		toSerialize["organisation"] = o.Organisation
	}
	return toSerialize, nil
}

func (o *Nis2Gap) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"requirement",
		"annex",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varNis2Gap := _Nis2Gap{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varNis2Gap)

	if err != nil {
		return err
	}

	*o = Nis2Gap(varNis2Gap)

	return err
}

type NullableNis2Gap struct {
	value *Nis2Gap
	isSet bool
}

func (v NullableNis2Gap) Get() *Nis2Gap {
	return v.value
}

func (v *NullableNis2Gap) Set(val *Nis2Gap) {
	v.value = val
	v.isSet = true
}

func (v NullableNis2Gap) IsSet() bool {
	return v.isSet
}

func (v *NullableNis2Gap) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNis2Gap(val *Nis2Gap) *NullableNis2Gap {
	return &NullableNis2Gap{value: val, isSet: true}
}

func (v NullableNis2Gap) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNis2Gap) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


